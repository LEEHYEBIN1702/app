<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>class.jsp</title>
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>
<style>
	.menu { border: 1px solid; background-color: gray; }
	.active { background-color:yellowgreen; }
</style>
<script>
$(function(){
	//li 태그에 click하면 태그 내용을 alert로 보이도록
	$("li.menu").on("click", function(){
	//	alert( $(this).html() ); //제이쿼리 방식
	//	alert( this.innerHTML ); //자바스크립트 방식
	$("li.menu").removeClass("active"); // 메뉴판 선택 할 때 많이 씀
	$(this).addClass("active");
	});
});
</script>
</head>
<body>
<ul>
   <li class="menu" data-num="1">게시판</li>
   <li class="menu" data-num="2">회원가입</li>
   <li class="menu active" data-num="3">로그인</li>
   <li class="menu" data-num="4">Qna</li>
</ul>
</body>
</html>